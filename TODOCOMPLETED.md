...
Acceptance Criteria:
- [x] Add an appointments tab or section in `src/frontend/src/pages/ClientPortal.tsx`.
- [x] Surface available appointment types and slots from `/api/portal/appointments/available-types/` and `/api/portal/appointments/available-slots/`.
- [x] Add booking and cancellation actions tied to portal endpoints.
- [x] Include tests for happy path booking, empty availability, and error responses.
- [x] Update portal UX coverage doc with new appointment flow coverage.
References:
- docs/03-reference/portal-ux-coverage.md
- src/frontend/src/pages/ClientPortal.tsx
- src/api/portal/urls.py
...
Acceptance Criteria:
- [x] Integrate web-vitals library in frontend.
- [x] Send metrics to analytics service (Sentry, Google Analytics, or custom).
- [x] Create dashboard showing: LCP, FID, CLS, TTFB, FCP, TTI.
- [x] Set alert thresholds: LCP <2.5s, FID <100ms, CLS <0.1.
- [x] Document in OBSERVABILITY.md.
References:
- src/frontend/
...
Acceptance Criteria:
- [x] Create config/database.py with query timeout configuration
- [x] Set PostgreSQL statement_timeout = 5s
- [x] Enable query logging for slow queries (> 100ms)
- [x] Add middleware for query timeout monitoring
- [x] Document timeout behavior and troubleshooting
References:
...
- [x] Add git-secrets hook to prevent secret commits
- [x] Add eslint for frontend files in src/frontend/
- [x] Test pre-commit hooks: try committing with lint error, verify blocked (pre-commit run attempted; see verification)
- [x] Document pre-commit setup in CONTRIBUTING.md
References:
- REFACTOR_PLAN.md:197-201, 441-478
- FORENSIC_AUDIT.md Issue #8.4
