# ConsultantPro — Governance-Aligned Task List

**Task Truth Source:** TODO.md
**Schema:** Each task includes ID, Priority, Type, Title, Owner (AGENT or Trevor), Context, Acceptance Criteria, References, Dependencies, Effort.
...
- ID: T-017
  Priority: P1
  Type: HYGIENE
  Title: Normalize legacy roadmap entries into the governance task format
  Owner: AGENT
  Context: The roadmap pre-dates the governance-aligned task template and requires normalization for consistency.
  Acceptance Criteria:
...
- ID: SEC-6
  Priority: P1
  Type: COMPLETE
  Title: Require webhook signature verification for DocuSign and Twilio
  Owner: AGENT
  Context: Webhook handlers allow unsigned requests when secrets are missing.
  Acceptance Criteria:
...
- ID: SEC-7
  Priority: P1
  Type: ENHANCE
  Title: Move auth tokens out of localStorage
  Owner: AGENT
  Context: localStorage tokens are vulnerable to XSS theft.
  Acceptance Criteria:
...
- ID: LLM-1
  Priority: P2
  Type: RESEARCH
  Title: Implement firm-scoped LLM client with safety controls
  Owner: AGENT
  Context: Safe GPT usage requires filters, metering, and auditability before rollout.
  Acceptance Criteria:
...
- ID: LLM-2
  Priority: P2
  Type: FEATURE
  Title: Add meeting prep background job with caching and fallbacks
  Owner: AGENT
  Context: Meeting prep automation depends on reliable asynchronous generation with cache safety.
  Acceptance Criteria:
