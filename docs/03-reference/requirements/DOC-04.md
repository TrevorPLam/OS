...
> **⚠️ SUPERSEDED**: This ADR has been superseded by ADR-0010. The actual tenancy model is firm-scoped row-level isolation, not schema-per-tenant. See ADR-0010 for the canonical decision.
...
- **ACTUAL**: Each firm is isolated using row-level scoping with `firm` ForeignKey on all models.
...
## ADR-0011: Immutability enforcement for legal and billing records
Status: Accepted
Date: 2025-12-29
Owners: Platform
Related: SYSTEM_SPEC.md; BILLING_LEDGER_SPEC.md; DOMAIN_MODEL.md

Context
- Signed contracts and invoiced time entries are legal evidence and billing sources of truth.
- Post-signature modifications to contracts or post-invoicing edits to time entries create audit gaps and potential disputes.
- Platform must prevent accidental or malicious tampering with finalized business records.

Decision
- Enforce model-level immutability constraints for critical business records:
  1. **Signed Contracts**: Once `signed_date` and `signed_by` are set, they cannot be modified
  2. **Proposal Amount Freeze**: Once a proposal has an associated signed contract, its `total_value` cannot be changed
  3. **Invoiced Time Entries**: Once a time entry is marked as `invoiced=True`, core fields (hours, rate, date) are locked

Consequences
Positive:
- Prevents data integrity violations that could undermine legal defensibility
- Enforces business rules at the model layer (defense in depth)
- Provides clear error messages when invalid modifications are attempted
- Supports audit and compliance requirements (Tier 3)

Negative / tradeoffs:
- Requires corrections to be handled through compensating transactions (new records) rather than edits
- May require admin override workflows for legitimate corrections (future enhancement)
- Slightly more complex data model with validation overhead

Risks:
- Edge cases where legitimate corrections are needed must be handled through documented processes
- Staff training required to understand immutability constraints

Alternatives considered
- Application-level validation only: rejected due to weaker guarantees (can be bypassed via shell, direct DB access)
- No immutability enforcement: rejected due to audit and legal risk
- Soft-lock with admin override: deferred to future implementation (would require break-glass audit logging)

Implementation
- `src/modules/crm/models.py`: Contract model `clean()` method validates signed field immutability
- `src/modules/crm/models.py`: Proposal model `clean()` method prevents amount changes when contract exists
- `src/modules/projects/models.py`: TimeEntry model `save()` method blocks edits to invoiced entries
- Tests added/updated to validate immutability constraints

Spec impact
- SYSTEM_SPEC.md: Immutability invariants for legal and billing records
- DOMAIN_MODEL.md: Contract, Proposal, and TimeEntry model constraints
- BILLING_LEDGER_SPEC.md: Invoiced time entry immutability requirement

---

## ADR-0010: Tenancy model is firm-scoped row-level isolation (not schema-per-tenant)
Status: Accepted  
Date: 2025-12-29  
Owners: Platform  
...
Decision  
- **The canonical tenancy model is firm-scoped row-level isolation**, not schema-per-tenant.
- All relevant documentation must be updated to reflect this reality.
- ADR-0001 is hereby **superseded** by this ADR.
- Schema-per-tenant MAY be reconsidered in the future for enterprise/regulated customers requiring physical isolation, but is NOT the current implementation.

Rationale for Row-Level Tenancy (as implemented):
1. **Operational simplicity**: Single schema simplifies migrations, backups, queries, and monitoring.
2. **Django ecosystem fit**: Row-level tenancy is idiomatic for Django ORM; schema switching adds complexity.
3. **Query enforcement**: FirmScopedQuerySet and permission middleware enforce isolation at query layer.
4. **Current scale**: SMB SaaS workloads do not require physical schema isolation.
5. **Proven pattern**: All existing code, tests, and deployments use row-level isolation.

When schema-per-tenant should be reconsidered:
- Enterprise customers requiring compliance certifications (SOC2 Type II dedicated environment)
- Regulated industries with contractual data residency requirements
- Customers requiring dedicated encryption keys per schema

Consequences  
Positive:
- Documentation now matches reality.
- No migration required; current implementation is correct.
- Simpler operational model for the current product stage.

Negative / tradeoffs:
- Row-level tenancy has higher blast radius for tenant leakage bugs (mitigated by FirmScopedQuerySet).
- Cross-tenant analytics are easier but require explicit authorization.

Migration path to schema-per-tenant (if needed later):
1. Introduce django-tenants or custom schema routing middleware.
2. Add schema provisioning workflow to onboarding.
3. Migrate data per-firm to dedicated schemas.
