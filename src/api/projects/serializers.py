...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Project
        fields = [
            'id', 'client', 'client_name', 'contract', 'contract_number', 'project_manager',
            'project_code', 'name', 'description', 'status', 'billing_type', 'budget',
            'hourly_rate', 'start_date', 'end_date', 'actual_completion_date',
            'created_at', 'updated_at', 'notes', 'total_hours_logged', 'total_billed_amount'
        ]
    ...
    def get_total_billed_amount(self, obj):
        ...
        total = obj.time_entries.filter(is_billable=True).aggregate(
    ...
    def validate(self, attrs):
        ...
        if contract and client and contract.client_id != client.id:
            raise serializers.ValidationError({
                'contract': 'Contract must belong to the same client as the project'
            })
...
class TimeEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TimeEntry
        fields = [
            'id', 'project', 'project_name', 'task', 'task_title', 'user', 'user_name',
            'date', 'hours', 'description', 'is_billable', 'hourly_rate', 'billed_amount',
            'invoiced', 'invoice', 'created_at', 'updated_at'
        ]
