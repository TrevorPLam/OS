...
from .models import Folder, Document, DocumentContent, Version, VersionContent
...
class DocumentContentAdmin(admin.ModelAdmin):
    list_display = ['document', 's3_bucket', 's3_key']
    search_fields = ['document__name', 's3_key']


@admin.register(VersionContent)
class VersionContentAdmin(admin.ModelAdmin):
    list_display = ['version', 's3_bucket', 's3_key']
    search_fields = ['version__document__name', 's3_key']
