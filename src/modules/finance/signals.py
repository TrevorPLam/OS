    ...
    if instance.pk:  # Only for updates
        try:
            ...
            if old_instance.status == 'paid':
                if (old_instance.total_amount != instance.total_amount or
                    old_instance.amount_paid != instance.amount_paid):
                    logger.error(
                        f"Attempted modification of paid bill {instance.reference_number}"
                    )
                    raise ValidationError(
                        "Cannot modify paid bills. Contact finance to issue adjustment."
                    )

            # Track status changes
            if old_instance.status != instance.status:
                logger.info(
                    f"Bill {instance.reference_number} status changed: "
                ...
                if old_instance.status in valid_transitions:
                    if instance.status not in valid_transitions[old_instance.status]:
                        logger.error(
                            f"Invalid state transition: {old_instance.status} → {instance.status}"
                        )
                        raise ValidationError(
                ...
                if instance.status == 'validated':
                    ...
                    if not instance.validated_by:
                        logger.warning(
                            f"Bill {instance.reference_number} validated without validator set"
                        )
                ...
                if instance.status == 'approved':
                    ...
                    if not instance.approved_by:
                        logger.warning(
                            f"Bill {instance.reference_number} approved without approver set"
                        )

                    logger.info(
                        f"✅ Bill {instance.reference_number} approved for payment: "
                ...
                if instance.status == 'scheduled':
                    if not instance.scheduled_payment_date:
                        ...
                        logger.info(
                            f"Auto-set scheduled_payment_date to due_date for bill {instance.reference_number}"
                        )
                ...
                if instance.status == 'paid':
                    ...
                    logger.info(
                        f"💰 Bill {instance.reference_number} paid: ${instance.amount_paid}"
                    )
                ...
                if old_instance.status == 'validated' and instance.status not in ['validated', 'approved', 'scheduled', 'paid']:
    ...
    if not created:
        try:
            if instance.status == 'validated':
                ...
                logger.info(
                    f"📅 Bill scheduled: {instance.reference_number} "
                    f"payment on {instance.scheduled_payment_date}"
                )
                ...
                logger.info(
                    f"💰 Bill paid: {instance.reference_number} "
                    f"${instance.amount_paid} to {instance.vendor_name}"
                )
                ...
                logger.info(
                    f"❌ Bill rejected: {instance.reference_number}"
                )
...
def approve_bill(bill, approver):
    ...
    if bill.status not in ['validated', 'disputed']:
        raise ValidationError(
            f"Cannot approve bill with status '{bill.status}'. "
            "Bill must be validated before approval."
        )
    ...
    logger.info(
        f"Bill {bill.reference_number} approved by {approver.username}: "
        f"${bill.total_amount} scheduled for payment"
    )
...
def schedule_bill_payment(bill, payment_date):
    ...
    if bill.status != 'approved':
        raise ValidationError(
            f"Cannot schedule bill with status '{bill.status}'. "
            "Bill must be approved before scheduling."
        )
    ...
    logger.info(
        f"Bill {bill.reference_number} scheduled for payment on {payment_date}: "
        f"${bill.total_amount}"
    )
...
def reject_bill(bill, rejector, reason=''):
    ...
    if bill.status in ['paid', 'partial']:
        raise ValidationError(
            f"Cannot reject bill with status '{bill.status}'. "
            "Paid bills cannot be rejected."
        )
    ...
    logger.info(
        f"Bill {bill.reference_number} rejected by {rejector.username}: "
        f"${bill.total_amount}. Reason: {reason}"
    )
