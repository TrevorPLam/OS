...
class AssetAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Asset Information', {
            'fields': ('asset_tag', 'name', 'description', 'category', 'status', 'assigned_to')
        }),
        ('Financial', {
            'fields': ('purchase_price', 'purchase_date', 'vendor', 'useful_life_years', 'salvage_value')
        }),
        ('Physical Details', {
            'fields': ('manufacturer', 'model_number', 'serial_number', 'location')
        }),
        ('Warranty', {
            'fields': ('warranty_expiration',)
        }),
        ('Notes', {
            'fields': ('notes',)
        }),
...
class MaintenanceLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Service Provider', {
            'fields': ('performed_by', 'vendor', 'cost')
        }),
        ('Audit', {
